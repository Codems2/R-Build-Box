-- ===========================================================================
-- Eliminar UNA sola sesión de una clase recurrente
--
-- Una clase recurrente es una única fila de schedule_slots que se repite cada
-- semana. Hasta ahora borrarla la quitaba de todas las semanas. Ahora el admin
-- puede elegir: eliminar solo la sesión de un día concreto (se guarda una
-- «excepción» para esa fecha) o eliminar la clase entera.
--
-- Las excepciones las ve todo el mundo (hacen falta para pintar el horario);
-- solo el admin las crea o las quita.
-- ===========================================================================
...
create table if not exists public.slot_exceptions (
  ...
  primary key (slot_id, class_date)
);

create index if not exists slot_exceptions_date_idx
  on public.slot_exceptions (class_date);

alter table public.slot_exceptions enable row level security;

drop policy if exists "Cualquiera ve las sesiones canceladas" on public.slot_exceptions;
create policy "Cualquiera ve las sesiones canceladas"
  on public.slot_exceptions for select
  using (true);

drop policy if exists "Solo el admin gestiona las sesiones canceladas" on public.slot_exceptions;
create policy "Solo el admin gestiona las sesiones canceladas"
  on public.slot_exceptions for all
  to authenticated
  using (public.is_admin())
  with check (public.is_admin());

grant select on public.slot_exceptions to anon, authenticated;
grant insert, update, delete on public.slot_exceptions to authenticated;
grant all on public.slot_exceptions to service_role;

-- ---------------------------------------------------------------------------
-- Eliminar la sesión de un día concreto
--
-- Recurrente  -> guarda la excepción de esa fecha y cancela sus reservas.
-- Puntual     -> equivale a borrar la clase (solo existe ese día).
-- Devuelve cuántas reservas se han eliminado, para poder avisar al admin.
-- ---------------------------------------------------------------------------
create or replace function public.delete_class_occurrence(
  p_slot_id uuid,
  p_class_date date
)
returns json
...
begin
  ...
  -- La fecha tiene que ser realmente una sesión de este hueco
  ...
  delete from public.bookings where slot_id = p_slot_id and class_date = p_class_date;
  get diagnostics v_removed = row_count;

  if v_slot.is_recurring then
    insert into public.slot_exceptions (slot_id, class_date)
      values (p_slot_id, p_class_date)
      on conflict do nothing;
  else
    delete from public.schedule_slots where id = p_slot_id;
  end if;

  return json_build_object(
    'ok', true,
    'bookings_removed', v_removed,
    'slot_deleted', not v_slot.is_recurring
  );
end;
$$;

grant execute on function public.delete_class_occurrence(uuid, date) to authenticated;

-- ---------------------------------------------------------------------------
-- book_class: no se puede reservar una sesión cancelada (CLASS_CANCELLED)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- book_guest: tampoco se puede apuntar a un invitado a una sesión cancelada
-- ---------------------------------------------------------------------------
